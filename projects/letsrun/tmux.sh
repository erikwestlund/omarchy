...
tmux send-keys -t $SESSION:$TABNO "if [ -s \"\$HOME/.nvm/nvm.sh\" ]; then . \"\$HOME/.nvm/nvm.sh\"; [ -f .nvmrc ] && nvm use >/dev/null; npm run dev; elif command -v mise >/dev/null 2>&1 && [ -f .nvmrc ]; then NODE_VERSION=\$(tr -d '[:space:]' < .nvmrc); mise exec \"node@\${NODE_VERSION}\" -- npm run dev; else npm run dev; fi" C-m
TABNO=$((TABNO+1))
