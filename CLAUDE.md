...
├── config/           # Config dirs/files → ~/.config/ (symlink or copy, see below)
...
### Currently Symlinked
- `home/` dotfiles → `~/` (individual files)
- `config/` directories: `darkman`, `ghostty`, `omarchy`, `projects`, `systemd`, `wireplumber`, `xdg-desktop-portal`
- `config/starship.toml` → `~/.config/starship.toml`
- `config/applications/*.desktop` → `~/.local/share/applications/` (individual files)

### Currently Copied (require ansible processing)
- `config/hypr/` - Templates `looknfeel-host.conf`, scripts need mode 0755
- `config/waybar/` - Templates `waybar-host.css`, `config.jsonc` modified via replace tasks
- `home/.ssh/config` - Needs mode 0600
- `home/bin/` → `~/.bin/` - Needs mode 0755
- `local/share/darkman/` - Scripts need mode 0755
- `system/` - Needs sudo, different ownership
- Templated files (Jinja2)
- Inline content generated by ansible
